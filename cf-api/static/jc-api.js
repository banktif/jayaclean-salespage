...
// Usage: var JC = JCApi.create('https://api.your-worker.workers.dev');
...
(function() {
  ...
  QueryBuilder.prototype._wrap = function(d, table) {
    // Make response compatible with Supabase format
    if (d.status === 'ok') {
      return {
    ...
    if (this._single && Array.isArray(d.data) && d.data.length === 1) {
      return { data: d.data[0], error: null };
    }
